  ...
  useEffect(() => {
    ...
    const fetchConversationsList = async () => {
      ...
      const list = convs && convs.length > 0 ? convs : [
        { id: "d0eebc99-9c0b-4ef8-bb6d-6bb9bd380a99", title: "Khách Cư Dân (Căn S2.05)", avatar: "🏢" },
        { id: "d0eebc99-9c0b-4ef8-bb6d-6bb9bd380a77", title: "Khách Cư Dân (Căn S1.02)", avatar: "🏠" },
      ];
      ...
      const formatted = await Promise.all(
        list.map(async (conv) => {
          ...
          return {
            ...conv,
            title: conv.title || "Khách Cư Dân (Căn S2.05)",
    ...
    const channel = supabase.channel(`provider_chat_list_realtime_${Math.random()}`);
    channel
      .on(
        "postgres_changes",
  ...
  return (
    <div className="max-w-md mx-auto bg-slate-50 min-h-screen flex flex-col justify-between shadow-2xl relative">
      <div className="bg-white px-4 py-3 border-b border-slate-200 sticky top-0 z-20 shadow-xs">
        <h1 className="text-sm font-bold text-slate-800">Danh sách Khách hàng (Thợ Tuấn)</h1>
      </div>
          ...
          conversations.map((item) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                {/* HÀNG 1: TÊN NICK KHÁCH HÀNG + THỜI GIAN */}
              ...
              {item.unread > 0 && (
                <span className="bg-blue-600 text-white text-[10px] font-extrabold px-2 py-0.5 rounded-full shadow-xs shrink-0 animate-pulse ml-1">
